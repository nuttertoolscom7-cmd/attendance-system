...
.stApp {
    ...
    background:
        linear-gradient(120deg, rgba(255,255,255,0.95), rgba(246,248,251,0.92)),
...
.stApp::before {
    ...
    pointer-events: none;
    background:
        linear-gradient(90deg, rgba(14, 116, 144, 0.10), transparent 28%),
        linear-gradient(180deg, rgba(15, 23, 42, 0.06), transparent 38%);
    z-index: 0;
}
...
.login-badge {
    ...
    border: 1px solid rgba(112, 128, 150, 0.24);
    border-radius: 999px;
    background: rgba(255, 255, 255, 0.72);
    color: #3c4a5f;
    font-size: 0.9rem;
    font-weight: 600;
    box-shadow: 0 10px 28px rgba(35, 48, 72, 0.08);
...
div[data-testid="stForm"] {
    max-width: 420px;
    margin: 0 auto;
    padding: 1.65rem 1.45rem 1.4rem 1.45rem;
    border: 1px solid rgba(160, 174, 195, 0.34);
    border-radius: 8px;
    background: rgba(255, 255, 255, 0.86);
    box-shadow: 0 22px 60px rgba(25, 38, 64, 0.14);
...
def get_auth_credentials():
    try:
        ...
        auth_config = {}
    username = os.getenv("ATTENDANCE_USERNAME") or auth_config.get("username", DEFAULT_AUTH_USERNAME)
    password = os.getenv("ATTENDANCE_PASSWORD") or auth_config.get("password")
    is_configured = bool(password)
    return str(username), str(password or ""), is_configured


def check_login(username, password):
    valid_username, valid_password, is_configured = get_auth_credentials()
...
def require_login():
    ...
    st.markdown(
        ...
        <section class="login-hero">
            <div class="login-badge">HR Attendance</div>
            <h1 class="login-title">ระบบเข้า-ออกงาน</h1>
            <p class="login-subtitle">เข้าสู่ระบบเพื่อจัดการข้อมูลการเข้างาน รายงานประจำวัน และสรุปรายเดือน</p>
        </section>
        """,
        unsafe_allow_html=True,
    )

    _, _, is_configured = get_auth_credentials()

    if not is_configured:
        st.error("ยังไม่ได้ตั้งค่ารหัสผ่านสำหรับระบบ")
        st.code(
            '[auth]\nusername = "admin"\npassword = "your-password"',
            language="toml",
        )
        st.caption("ตั้งค่าใน .streamlit/secrets.toml หรือใช้ ATTENDANCE_USERNAME และ ATTENDANCE_PASSWORD")
        st.stop()
    ...
    with center:
        ...
        st.markdown(
            '<p class="login-footnote">ระบบนี้จัดทำขึ้นโดย นายเอกชัย ไตรโยธี</p>',
            unsafe_allow_html=True,
        )
...
# -------------------------
with st.sidebar:
    st.success(f"เข้าสู่ระบบ: {st.session_state.get('auth_username', '')}")
