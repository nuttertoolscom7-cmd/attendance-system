...
def normalize_whitespace(s: str) -> str:
    if s is None: return ""
    # 1. ล้างตัวอักษรล่องหนและวงกลมประ
    s = re.sub(r'[\u200b\ufeff\xa0\u25cc]', '', s)
    
    # 2. ลบช่องว่างที่อยู่หน้าสระ/วรรณยุกต์ (ที่มักแทรกมาใน PDF)
    s = re.sub(r'\s+([\u0E31\u0E34-\u0E3A\u0E47-\u0E4E])', r'\1', s)
    
    # 3. **ระบบ Re-ordering**: สลับลำดับ สระข้าง (ะ า ำ) กับ วรรณยุกต์ (่ ้ ๊ ๋ และ ็)
    # หากสระข้างมาก่อนวรรณยุกต์ (ผิดหลัก) ให้สลับเอาวรรณยุกต์มาไว้หน้าสระข้าง
    # ช่วงสระข้าง: [\u0E30\u0E32\u0E33]
    # ช่วงวรรณยุกต์และไม้ไต่คู้: [\u0E47-\u0E4B]
    s = re.sub(r'([\u0E30\u0E32\u0E33])([\u0E47-\u0E4B])', r'\2\1', s)

    # 4. จัดลำดับกรณี วรรณยุกต์สลับกับสระบน/ล่าง
...
if 'master_df' in st.session_state:
    ...
    with tab3:
        st.subheader("🏢 การมาสายแยกตามแผนก")
